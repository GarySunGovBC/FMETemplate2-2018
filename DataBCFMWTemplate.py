...
class CalcParamsBase(GetPublishedParams):
    ...
    def __init__(self, fmeMacroVals):
        ...
        self.logger.debug("calcparams base completed")
    ...
    def getSrcSSDatabaseConnectionFilePath(self, position=None):
        '''
        returns the database connection file path for a sql server
        source database.  Also if the connection file does not
        exist it gets generated by the plugin.

        This functionality is forked. in dev mode the script expects
        the sde file to exist.  In prod mode it will attempt to create
        the connection file if it doesnt already exist.
        '''
        if not self.plugin:
            self.addPlugin()
        msg = self.debugMethodMessage.format(
        ...
        self.logger.debug(msg)
        srcConnFilePath = self.plugin.getSrcSSDatabaseConnectionFilePath(
            position)
...
class CalcParamsDevelopment(object):
    ...
    def getFailedFeaturesFile(self, failedFeatsFileName=None, postion=None):
        ...
        if not os.path.exists(ffDir):
            msg = 'The failed features directory %s does not exist, ' + \
                  'creating now'
    ...
    def getDestDatabaseConnectionFilePath(self, position=None):  # @IgnorePep8
        ...
        if not os.path.exists(connectionFileFullPath):
            msg = 'Looking for a destination connection file with the ' + \
                  'name {0}. This file does not exist.  Please create it' + \
                  '  using arccatalog and then re-run this job'
    ...
    def getSrcDatabaseConnectionFilePath(self, position=None):
        ...
        if not os.path.exists(connectionFileFullPath):
            msg = 'Looking for a destination connection file with the ' + \
                  'name {0}.  This file does not exist.  Please create ' + \
                  'it using arccatalog and then re-run this job'
            msg = msg.format(connectionFileFullPath)
            self.logger.error(msg)
            raise IOError(msg)
        else:
    ...
    def getSrcSSDatabaseConnectionFilePath(self, position=None):
        '''
        when running on a non databc machine the framework expects the
        connection file to be user created.  method checks to see if the
        expected path / file exists and if it does not  raises an error
        allerting user to requirement.
        '''
        destDir = self.parent.getFMWDirectory()
        host = self.parent.getSrcHost(position)
        serviceName = self.parent.getSqlServerDatabaseName(position)
        fileNameTmpl = '{0}__{1}.sde'
        connectionFile = fileNameTmpl.format(host, serviceName)
        connectionFileFullPath = os.path.join(destDir, connectionFile)
        if not os.path.exists(connectionFileFullPath):
...
class CalcParamsDataBC(object):
    ...
    def getSrcSSDatabaseConnectionFilePath(self, position):
        '''
        ...
        This method returns the path to the SDE connection file for a sql
        server source type.  If the connection file does not already exist
        it will get created.  The parameters required to create the
        connection file come from the parameters:
          - SRC_HOST
          - SRC_SS_DBNAME
          - SRC_PORT.
        '''
        ...
        dbName = self.parent.getSrcSqlServerDatabaseName(position)
        self.logger.debug("src port: %s", port)
        msg = 'In order to calculate the name of the source database ' + \
              'connection for a sql server sde the parameter {0} must ' + \
              'be defined as a published parameter in the fmw.  Currently' + \
              ' it is not.  Define this parameter and re-run'
        if not host:
            ...
            raise IOError(msg)
        if not dbName:
            msg = msg.format(self.const.FMWParams_SrcSSDbName,
                             'source Sql Server Database Name')
            self.logger.error(msg)
            raise IOError(msg)
        connFileName = '{0}__{1}.sde'.format(host, dbName)
    ...
    def getSrcDatabaseConnectionFilePath(self, position=None):
        ...
        msg = 'In order to calculate the name of the source database ' + \
              'connection the {1} parameter: {0} must be defined as a ' + \
              'published parameter in the fmw.  Currently it is not ' + \
              'Define the parameter and re-run'
        ...
        if not host:
            ...
            raise IOError(msg)
        if not servName:
            msg = msg.format(self.const.FMWParams_SrcServiceName,
                             'source service name')
            self.logger.error(msg)
            raise IOError(msg)
...
#                                   serviceName, port)
    ...
    def __createSDEConnectionFile(self, connectionFileFullPath, host,
                                  serviceName, port=None, dbType='oracle'):
        '''
        ...
        :param connectionFileFullPath: Path to the esri .sde connection file
                                       that can be used to connect to the
                                       database in question.
        :param host: the host name used to create the connection
        :param serviceName: for oracle connections this is the servicename,
                            for sql server put the database name in here
        :param port: the port used to connect to the database.
        :param dbType:  {oracle|sqlserver}, defines the type of esri
                       direct connect string to generate, that in turn is
        ...
        if snList:
            if len(snList) > 1:
                ...
                if len(pswdList) > 1:
                    ...
                    msg = 'Looking for the password for the schema {0}, ' + \
                          'and service name {1}  Found the following ' + \
                          'accounts that roughly match that combination ' + \
                          '{2} pmp has different passwords for each ' + \
                          'of these.  Unable to proceed as not sure ' + \
                          'which password to use.  Fix this by changing ' + \
                          'the parameter to match a "User Account" in ' + \
                          'pmp exactly.'
...
class ModuleLogConfig(object):
    ...
    def __init__(self, fmwDir, fmwName, destKey=None, customLogConfig=None):
        ...
        tmpLog.debug("logFileFullPath: %s", enhancedLoggingFullPath)
...
#             logger.info("log config file being used: %s", logConfFileFullPath)
        ...
        # and finnally double check that its all set up correctly and fme
    ...
    def getEnhancedLoggerPath(self):
        ...
        code.
        '''
        enhancedLoggingFileName = Util.calcEnhancedLoggingFileName(self.fmwName)
        enhancedLoggingDir = \
    ...
    def getDatabaseConnection(self):
        ...
        try:
            ...
            try:
                ...
                # self.logger.info(msg)
                msg = u'database connection used to write to DWM has ' + \
                      u'failed, dwm record for this replication will ' + \
                      u'not be written'
                self.logger.error(msg)
                raise
        else:
