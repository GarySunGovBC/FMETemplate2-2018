...
:description:  Retrieves application level parameters from a config 
               file.  implements a very simple api that should make 
               it trivial to retrieve these parameters.
   


Dependencies:
-------------------
Uses the ConfigParser module to read from a config file.
...
class Config():
    '''
    Provides an api for retrieving application level 
    properties / parameters.  Parameter are stored in 
    a config file, however once the application is properly
    installed and configured you shouldn't even need to 
    know this.
